...
- [~] **T/W: Prosa/Inhalt pro Kapitel spliten.** Bei ~400 Seiten darf die `index.html` nicht Monolith bleiben (sonst Zehntausende Zeilen, nicht in einem Context editierbar). Pro Kapitel ein HTML-Fragment (`chapters/ch_NN.html`), per `core.js` nachgeladen oder per einfachem Build zusammengefügt. *Zwingend für Editierbarkeit bei Skalierung.* — *Teilweise (v2.0): `src/pages.js` gruppiert `#paper` zur Laufzeit in Ein-Abschnitt-pro-Seite-Einheiten (Kapitel-App-Shell, s. CLAUDE.md) — der Content-Modell-Teil dieses Items ist erledigt, die eigentliche Datei-Fragmentierung (`chapters/ch_NN.html`) bleibt offen, da noch kein zweites Kapitel ansteht.*
