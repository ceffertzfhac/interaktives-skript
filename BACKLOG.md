...
- [ ] **T/W: Prosa/Inhalt pro Kapitel spliten.** Bei ~400 Seiten darf die `index.html` nicht Monolith bleiben (sonst Zehntausende Zeilen, nicht in einem Context editierbar). Pro Kapitel ein HTML-Fragment (`chapters/ch_NN.html`), per `core.js` nachgeladen oder per einfachem Build zusammengefügt. *Zwingend für Editierbarkeit bei Skalierung.* — *P1b (asynchroner Kapitel-Split folgt separat).*
