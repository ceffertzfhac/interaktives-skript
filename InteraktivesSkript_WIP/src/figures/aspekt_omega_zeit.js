// aspekt_omega_zeit.js — interaktive Aspekt-Figur zu Abbildung 1.44 (1.4.2
// „Die Geschwindigkeit …", Winkelgeschwindigkeit-Zeit-Diagramm). Zeigt die
// Kreisbahn-Szene mit Ortsvektor r, durchlaufenem Bogen und dem Winkel φ (Bogen
// + φ-Label) sowie rechts daneben das ω(t)-Diagramm — ω ist bei konstanter
...
// selbst hat keine sinnvolle Bogen-Darstellung; die Szene bleibt also wie 1.41,
// nur das Diagramm daneben wechselt von φ(t) auf ω(t)). Der Motor der großen
// Kreisbewegungs-Simulation (src/figures/kreisbewegung/) zeichnet Szene
// (updateScene) UND Diagramm (updateGraph), feature-gated auf den ω-Aspekt.
// EINZELNES Diagramm mit graphType1='omega' (nicht gestapelt wie 1.39): der
// Motor liefert omega nativ (physics.js, konstant pro Zeitschritt). Der Motor
// ist zeit-/ω-getrieben (angleRad(t)=φ0+ω·t); der t-Regler steuert direkt die
...
// zeichnen daraus Punkt, Ortsvektor, Bahnspur, die (konstante) ω(t)-Linie — die
// optionale Anim-Schleife fährt die Zeit von 0 bis 12 s (Slow-Mo via Tempo-Pills).
//
// VORLAGE ist aspekt_winkel_zeit.js (Abb. 1.41) — 1:1 kopiert, nur graphType1
// und die Datenreihe der Vergleichslinie geändert (Regel 0a: „wie 1.38" heißt
// pixelgleich; hier analog „wie 1.41" für die komplette Szene).
//
...
// schwächer als der aktuelle. Der kumulierte Verlauf steht in 1.41s φ(t)-
// Diagramm; hier zeigt das Diagramm stattdessen die (konstante) Steigung ω.
//
...
// nicht ändern. Winkelbogen UND ω(t)-Kurve in --kb-accent (der φ/ω-Farbe, gleiche
// Größenfamilie), damit Szene und Diagramm zusammengehören.
//
// LAYOUT: anders als 1.39 (dort Szene über dem gestapelten Diagramm) steht hier
// die Kreisbahn-Szene NEBEN dem ω(t)-Diagramm (Zeile) — wie die statische Vorlage
// (Kreis links, Diagramm rechts). Nur im Schmal-Modus wird gestapelt
// (Szene über Diagramm); breit + Lupe ebenfalls Zeile (s. aspekt_omega_zeit.css).
//
...
//    kompakt und NUR auf diese Figur gemünzt (ω(t), φ — kein R, kein x/y),
  ...
  <div class="panel-body">
    ...
    <div class="panel-section">
      ...
      <div class="formula-box">
        <div class="formula-box-cap">Winkelgeschwindigkeit auf der Kreisbahn:</div>
        <div>\\[\\omega = \\dot{\\varphi} = \\tfrac{2\\pi}{T}\\]</div>
        <div>\\[\\varphi(t) = \\omega\\,t\\]</div>
      </div>
    </div>
  </div>
</div>`;
...
export function buildOmegaZeitFig(fig) {
    ...
    // -- Vergleichslinie (Ghost): die ω(t)-Linie des letzten Parametersatzes
    //    einfrieren (gestrichelt + dünner, s. aspekt_omega_zeit.css) und über
    ...
    //
    //    Gespeichert werden DATEN (t/ω), nicht Pixel: die y-Achse skaliert mit T
    //    (ω = 2π/T, also größer bei kleinerem T). Eine in Pixeln eingefrorene
    //    Polyline würde beim nächsten Parameterwechsel stehenbleiben, während
    //    die Achse unter ihr wegskaliert — die Geisterlinie schien sich dann
    //    "mitzuändern". Deshalb wird sie bei jedem Zeichnen aus den Rohdaten neu
    //    auf die AKTUELLE Achse projiziert.
    const prevLine = ge(p + 'graph_prev_line');
    let prevSeries = null;                // {t:[…], omega:[…]} — komplette Kurve
    function snapshotPrev() {             // inside withStore aufrufen
        prevSeries = store.tData.length
            ? { t: store.tData.slice(), omega: store.omegaData.slice() }
            : null;
    }
    function clearPrev() {
        prevSeries = null;
    ...
    function renderPrev() {
        ...
        for (let i = 0; i < prevSeries.t.length; i++) {
            ...
            const py = plotT + plotH - ((prevSeries.omega[i] - yMin) / yR) * plotH;
    ...
    function precomputeRange(duration) {
        ...
        // (kleineres T -> größeres ω) oben aus dem Diagramm heraus und der
        // Vergleich bricht ab.
        if (prevSeries && prevSeries.omega.length) {
            const lim = store.axisLimits.omega;
            const need = Math.max(...prevSeries.omega) * 1.1;
    ...
    function rebuild(paramChange = false) {
        rt.withStore(() => {
            ...
            Object.assign(store, {
                isStacked: false, graphType1: 'omega',
    ...
    function onInput(e) {
        ...
        rebuild(true);
        // War nicht der ω-Schieber die Quelle (T-Zug) -> ω-Schieber nachführen,
        // damit beide gekoppelt sichtbar bleiben.
        if (e.target !== ak_omega) {
