// aspekt_axay_zeit.js — interaktive Aspekt-Figur zu Abbildung 1.46 (1.4.6
// „Beschleunigung …", Bahnbeschleunigungs-Komponenten-Diagramm). Zeigt die
// beiden Komponenten-Zeit-Diagramme a_x(t) (oben) und a_y(t) (unten) ueber
// drei Perioden (0 … 12 s) sowie die Kreisbahn-Szene mit dem
// Beschleunigungsvektor \vec{a} (radial, antiparallel zum Ortsvektor) — und auf
// Wunsch dessen Komponenten a_x/a_y (Projektionen auf die Achsen), genau die
// Größen, deren Zeitverlauf die Diagramme zeigen.
...
// Diagramme (updateGraph), feature-gated auf den Beschleunigungs-Aspekt.
// Stacked-Modus mit graphType1='axt' / graphType2='ayt' (der Motor liefert
...
// Beschleunigungsvektor (+ optionale Komponenten), Bahnspur, die wachsenden
// Kurven UND die analoge Stoppuhr — die optionale Anim-Schleife faehrt die Zeit
// von 0 bis 12 s (Slow-Mo via Tempo-Pills).
//
// Vorlage ist aspekt_vxvy_zeit.js (Abb. 1.42, gestapelte Dual-Graphen + Auto-
// Stopp + Vergleichslinie): gleiche Interaktion, nur der Aspekt wechselt von
// Geschwindigkeit auf Beschleunigung. Was aus der Vorlage stammt, lief auf
// Anhieb; deshalb wurde kopiert und feature-gated, nicht neu erfunden
// (Runbook Abschnitt 0a).
//
// SZENE: Basis ist der radiale Beschleunigungsvektor \vec{a} (antiparallel zu
// \vec{r}, wie die statische Abb. 1.48). Die Komponentenzerlegung (a_x/a_y,
// Projektionen auf die Achsen — passend zu den beiden Diagrammen) ist OPTIONAL
// ueber den Umschalter „Komponenten zerlegen" im Bedien-Panel zuschaltbar
// (Nutzervorgabe, analog 1.42). Der Ortsvektor bleibt EIN (nicht AUS, wie in
// 1.42): a⃗ ist antiparallel zu r⃗, das sehen Nutzer nur, wenn r⃗ mit steht —
// das ist der paedagogische Punkt dieser Figur. Der Geschwindigkeitsvektor ist
// aus (Aspekt ist die Beschleunigung).
//
...
const T_AUTO = 12;            // fester Auto-Stopp nach 12 s — bei Vorgabe T=4 s
                              // sind das drei Perioden (Bereich 0…12 s, wie 1.39/1.41/1.42)
...
<div class="aspekt-panel aspekt-panel-left">
  ...
  <div class="panel-section">
    ...
    <label class="aspekt-check"><input type="checkbox" id="ak_components"><span>Komponenten \\(a_x\\)/\\(a_y\\) zerlegen</span></label>
  </div>
  <div class="panel-section">
...
export function buildAxAyZeitFig(fig) {
    ...
    let showComponents = false;           // Szene: a_x/a_y-Komponentenzerlegung (optional, Nutzervorgabe)
    ...
    // -- Zeitreihe auf feste 12 s begrenzen (3 Perioden bei T=4 s, wie 1.39/1.41/1.42).
    ...
    //    Laeuft inside withStore (liest store + acceleration(t)).
