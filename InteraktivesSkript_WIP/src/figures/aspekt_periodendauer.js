// aspekt_periodendauer.js — interaktive Aspekt-Figur zu Abbildung 1.49 (1.4.4
// „Periodendauer und Frequenz einer Kreisbewegung"). Baut auf der Weg-Zeit-
// Figur (1.39, aspekt_weg_zeit.js) auf — dieselben gestapelten x(t)/y(t)-
// Diagramme + Kreisbahn-Szene —, feature-gated auf den ABLESE-Aspekt der
// Periodendauer: im oberen x(t)-Diagramm sind ZWEI aufeinanderfolgende Maxima
// markiert (t=0 und t=T, x(t)=R·cos(ωt)) und die T-Strecke dazwischen als
// bemaßter Doppelpfeil eingezeichnet. So wird sichtbar, dass T der zeitliche
// Abstand zweier aufeinanderfolgender Maxima ist. Zusätzlich zeigt die Analyse
// f = 1/T und ω = 2π/T. Der R-Regler ändert nur die Amplitude, NICHT den
// Maxima-Abstand — das demonstriert, dass T unabhängig vom Radius ist.
...
<svg id="kb_graph_svg" viewBox="0 0 700 410" preserveAspectRatio="xMidYMid meet" class="aspekt-graph-svg">
  <defs>
    ...
    <!-- Bemaßungs-Pfeilspitze für die T-Strecke: orient="auto-start-reverse"
         dreht die Spitze am Linien-ANFANG um, sodass beide Enden nach AUSSEN
         zeigen (◄──T──►). Feste Länge (userSpaceOnUse), Füllung via .tmark-arrowhead. -->
    <marker id="kb_tmark_arrow" markerUnits="userSpaceOnUse" markerWidth="7" markerHeight="8" refX="6" refY="4" orient="auto-start-reverse"><polygon class="tmark-arrowhead" points="0 0.5, 6 4, 0 7.5"/></marker>
  </defs>
  ...
  <g id="kb_graph_group_stacked_top" transform="translate(0,0)" style="visibility:hidden">
    ...
    <!-- T-Ablese-Markierung: zwei Maxima-Punkte + Hilfslinien + bemaßte
         T-Strecke, pro Zeichnen aus store.graphScale.top projiziert (drawTMarkers). -->
    <g id="kb_tmark_top"></g>
  ...
  <div class="panel-body">
    <div class="panel-section">
      ...
      <div class="analysis-grid">
        ...
        <div class="analysis-cell key">Frequenz \\(f=\\tfrac{1}{T}\\)</div> <div class="analysis-cell val" id="ak_val_f"></div>
        <div class="analysis-cell key">Kreisfrequenz \\(\\omega\\)</div> <div class="analysis-cell val" id="ak_val_omega"></div>
    ...
    <div class="panel-section">
      ...
      <div class="formula-box">
        <div class="formula-box-cap">Periodendauer, Frequenz, Kreisfrequenz:</div>
        <div>\\[f = \\tfrac{1}{T}\\]</div>
        <div>\\[\\omega = \\tfrac{2\\pi}{T} = 2\\pi f\\]</div>
        <div class="formula-box-cap">\\(T\\) ist der Abstand zweier Maxima von \\(x(t)=R\\cos(\\omega t)\\).</div>
      </div>
    </div>
  </div>
</div>`;
...
export function buildPeriodendauerFig(fig) {
    ...
    //    durchlaufenen Winkelbogen, auch wenn der Aspekt hier x(t)/y(t) statt
    //    φ/ω ist): der AKTUELLE Umlauf wird als Teilbogen (0…φ, volle Opazität,
    ...
    function draw(t) {
        ...
        drawTMarkers(); // ebenfalls nach updateGraph (braucht store.graphScale.top)
    ...
    function updateLabels(t, T) {
        ...
        if (vt) {
            ...
            const vf = ge(p + 'ak_val_f'); if (vf) vf.textContent = n(1 / T, 3) + ' Hz';
            const vom = ge(p + 'ak_val_omega'); if (vom) vom.textContent = n(omegaOf(T), 3) + ' rad/s';
