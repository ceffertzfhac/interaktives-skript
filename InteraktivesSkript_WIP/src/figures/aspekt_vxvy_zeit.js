// aspekt_vxvy_zeit.js — interaktive Aspekt-Figur zu Abbildung 1.42 (1.4.5
// „Geschwindigkeit …", Bahngeschwindigkeits-Komponenten-Diagramm). Zeigt die
// beiden Komponenten-Zeit-Diagramme v_x(t) (oben) und v_y(t) (unten) ueber
// drei Perioden (0 … 12 s) sowie die Kreisbahn-Szene mit dem
// Geschwindigkeitsvektor \vec{v} (tangential) — und auf Wunsch dessen
// Komponenten v_x/v_y (Projektionen auf die Achsen), genau die Größen, deren
// Zeitverlauf die Diagramme zeigen.
...
// Diagramme (updateGraph), feature-gated auf den Geschwindigkeits-Aspekt.
// Stacked-Modus mit graphType1='vxt' / graphType2='vyt' (der Motor liefert
// beide nativ, physics.js). Der Motor ist zeit-/ω-getrieben
...
// Geschwindigkeitsvektor (+ optionale Komponenten), Bahnspur, die wachsenden
// Kurven UND die analoge Stoppuhr — die optionale Anim-Schleife faehrt die Zeit
// von 0 bis 12 s (Slow-Mo via Tempo-Pills).
//
// Vorlage ist aspekt_weg_zeit.js (gestapelte Dual-Graphen + Auto-Stopp +
// Vergleichslinie): gleiche Interaktion, nur der Aspekt wechselt von Ort auf
// Geschwindigkeit. Was aus der Vorlage stammt, lief auf Anhieb; deshalb wurde
// kopiert und feature-gated, nicht neu erfunden (Runbook Abschnitt 0a).
//
// SZENE: Basis ist der tangentiale Geschwindigkeitsvektor \vec{v} (wie die
// statische Abb. 1.45). Die Komponentenzerlegung (v_x/v_y, Projektionen auf die
// Achsen — passend zu den beiden Diagrammen) ist OPTIONAL über den Umschalter
// „Komponenten zerlegen" im Bedien-Panel zuschaltbar (Nutzervorgabe). Der
// Ortsvektor bleibt bewusst AUS (Aspekt ist die Geschwindigkeit, nicht der Ort)
// — die Szene entspricht damit der statischen Geschwindigkeits-Vorlage, nicht
// der Weg-Zeit-Szene.
//
...
// (v_x/v_y) + die Vergleichslinie + die Geschwindigkeits-Vektorfarben. Die
...
// den Zielpunkt setzt.
//
// LAYOUT: wie 1.39 — stackedDualGeom() des Motors stellt die beiden Diagramme
...
const T_AUTO = 12;            // fester Auto-Stopp nach 12 s — bei Vorgabe T=4 s
                              // sind das drei Perioden (Bereich 0…12 s, wie 1.39/1.41)
...
// -- Szene: Vorlagen-Geometrie (wie aspekt_weg_zeit), aber der Geschwindigkeits-
//    vektor ist die gezeigte Groesse — daher bekommt er (und seine Komponenten)
//    eigene Pfeilspitzen (v/vx/vy, Geometry = r/rx/ry: main 18.75 / comp 15) UND
//    marker-end auf den Linien (die Vorlage blendet v aus und braucht sie
//    nicht). Der Ortsvektor bleibt als Stub (updateScene() dereferenziert ihn),
//    ist aber per showPositionVector=false verdeckt. Versteckte Stubs (a),
...
<div class="aspekt-panel aspekt-panel-left">
  ...
  <div class="panel-section">
    ...
    <label class="aspekt-check"><input type="checkbox" id="ak_components"><span>Komponenten \\(v_x\\)/\\(v_y\\) zerlegen</span></label>
  </div>
  <div class="panel-section">
...
//    Physik-Sektion wie in 1.41: statischer .formula-box-Block (kein data-eqs),
  ...
  <div class="panel-body">
    <div class="panel-section">
      ...
      <div class="analysis-grid">
        ...
        <div class="analysis-cell key">Geschw. \\(v_x\\)</div>        <div class="analysis-cell val" id="ak_val_vx"></div>
        <div class="analysis-cell key">Geschw. \\(v_y\\)</div>        <div class="analysis-cell val" id="ak_val_vy"></div>
...
// ── Factory: baut EINE v_x/v_y-Zeit-Aspekt-Figur mit eigener Motor-Instanz ─────
export function buildVxVyZeitFig(fig) {
    ...
    let showComponents = false;           // Szene: v_x/v_y-Komponentenzerlegung (optional, Nutzervorgabe)

    // -- Vergleichslinie (Ghost): die gerade fertigen v_x(t)/v_y(t)-Kurven beim
    ...
    //
    //    Gespeichert werden DATEN (t/vx/vy), nicht Pixel: die y-Achsen skalieren
    ...
    function renderPrev() {
        const series = { vxt: prevSeries && prevSeries.vx, vyt: prevSeries && prevSeries.vy };
    ...
    //    durchlaufenen Winkelbogen, auch wenn der Aspekt hier vx/vy statt φ/ω
    ...
    function precomputeRange(duration) {
        ...
        if (prevSeries && prevSeries.t.length) {
            ...
            for (const [key, arr] of [['vxt', prevSeries.vx], ['vyt', prevSeries.vy]]) {
    ...
    //    Laeuft inside withStore (liest store + velocity(t)).
    function updateLabels(t, T) {
        ...
        if (vt) {
            ...
            const v = velocity(t);
            ge(p + 'ak_val_vx').textContent = n(v.x, 2) + ' m/s';
            ge(p + 'ak_val_vy').textContent = n(v.y, 2) + ' m/s';
            ge(p + 'ak_val_vabs').textContent = n(Math.hypot(v.x, v.y), 2) + ' m/s';
        }
    }
    ...
    function onInput(e) {
        ...
        // (Loslassen) beendet die Geste. (Fallstrick #18)
        if (!paramGesture) {
            paramGesture = true;
    ...
    // store.showVelocityComponents — der Motor zeichnet dann v_x/v_y als
    // Projektionen auf die Achsen. Rebuild nötig, weil updateScene das Flag nur
    // beim Zeichnen auswertet (kein separaten Teil-Draw nötig, draw() reicht,
    // aber das Flag muss vor dem nächsten draw gesetzt sein).
    if (ak_components) ak_components.addEventListener('change', () => {
        showComponents = ak_components.checked;
        rt.withStore(() => {
            store.showVelocityComponents = showComponents;
            draw(curT);
        });
    });
    ...
    //    Mausposition. Die Figur nutzt den stacked Modus (v_x(t) oben, v_y(t)
