// aspekt_omega_vektor.js — interaktive Aspekt-Figur zu Abbildung 1.57 (Abschnitt
// 1.4.8 „Winkelgeschwindigkeit als Vektor"). Die statische Abbildung besteht aus
// zwei Teilbildern: (a) Winkelgeschwindigkeit \vec\omega in POSITIVER z-Richtung
// (-> Bewegung gegen den Uhrzeigersinn) und (b) in NEGATIVER z-Richtung (-> im
// Uhrzeigersinn). Genau dieser Gegensatz ist der Aspekt: EIN Regler (\omega_0
// mit Vorzeichen) fasst beide Teilbilder zusammen, das \varphi(t)-Diagramm
// daneben zeigt die Folge (steigende Rampe bei \omega_0>0, fallende bei
// \omega_0<0 — gleiche Steigung, entgegengesetztes Vorzeichen = die zwei
// Drehrichtungen).
//
// GLEICHER MOTOR wie 1.59 (kreis_spiral, ISO-3D mit Drehachse): \vec\omega liegt
// auf der z-Achse und waere in der Draufsicht des Kreisbewegungs-Motors ein
// Punkt. Aufbau, Bedienung und Layout sind 1:1 von aspekt_alpha_omega.js (Abb.
// 1.59) uebernommen — dasselbe Interaktionsmuster (Szene NEBEN einem einzelnen
// Diagramm, Zeit-Regler + Start/Stop/Reset + Tempo-Pills + „Letzte Kurve
// behalten", Lupe, Analyse-Panel). Uebernommen wurde die Struktur 1:1; ersetzt
// sind nur die Regler (\omega_0 statt \omega_0+\alpha — hier ist \alpha=0 fix,
// die Bewegung ist gleichfoermig), die Szenen-Gates (zusaezlich \vec v
// tangential) und die Diagrammgroesse (\varphi(t) statt \omega(t)).
//
// UNTERSCHIED zu 1.59: hier ist die Winkelbeschleunigung \alpha stets null — die
// Bewegung ist gleichfoermig (\omega konstant). Damit entfaellt der \alpha-Regler
// und der \alpha-Vektor; \vec v = \vec\omega\times\vec r (tangential) kommt als
// dritter Vektor dazu. Das \varphi(t)-Diagramm zeigt eine GERADE mit der Steigung
// \omega_0 (Vorzeichen = Drehrichtung), ganz analog zu 1.59s \omega(t)-Gerade.
//
// PARAMETER: \omega_0 (Anfangs-Winkelgeschwindigkeit, auch 0/negativ) mit
// demselben Bereich wie 1.59 (\pm 2,0 rad/s, Schritt 0,05), damit die Regler
// kapitelweit gleich „schmecken". \omega_0 > 0 = Teilbild (a) gegen den
// Uhrzeigersinn, \omega_0 < 0 = Teilbild (b) im Uhrzeigersinn, \omega_0 = 0 =
// Stillstand (Grenzfall, \varphi bleibt konstant).
//
...
//     zwei Aspekte in eine Figur mischen.
//   * Zusaetzlich zu \vec\omega zeigt die Szene den Ortsvektor \vec r, die
//     Bahngeschwindigkeit \vec v, den durchlaufenen Bogen und den Winkel
//     \varphi (letztere drei abschaltbar). Die statische Abbildung zeigt nur
//     \vec\omega, \vec r und \vec v; der bewegte Punkt ist hier der eigentliche
//     Gewinn der Interaktion (man sieht die Drehrichtung LIVE).
//   * \vec v ist MASSSTAEBLICH gezeichnet (reale Laenge VEL_SCALE des Motors),
...
//     kollidieren lassen (s. --kb-omega/--kb-rlat/--kb-vlat).
//
// FARBEN: \omega blau, \vec r grau, \vec v orange — NICHT frei gewaehlt, sondern
// von der Bildunterschrift der statischen Abbildung vorgegeben („Winkelgeschwin-
// digkeit (in Blau)", „Ortsvektor (in Grau)", „Geschwindigkeit (in Orange)").
// Tokens --kb-omega/--kb-rlat/--kb-vlat (aspekt_kreisbahn.css) tragen die Werte
// der LaTeX-Quelle (\textcolor[HTML]{1555A2} bzw. {474747} bzw. {F47A2D}).
//
...
// \vec\omega [rad/s] liegt auf der Drehachse und hat keine gemeinsame Laengenska-
// la mit den in der Ebene liegenden Vektoren \vec r [m] und \vec v [m/s] — ein
// gemeinsamer Maßstab existiert physikalisch nicht. Daher ein eigener, LINEARER
// Maßstab fuer \vec\omega (kein log, keine Saettigung): doppelt so großes
// \omega_0 = doppelt so langer Pfeil. Uebernommen aus 1.59 (dieselbe Figur-Fami-
// lie, derselbe Reglerbereich -> kapitelweit gleiche \omega-Pfeillaenge).
//   omega: 0,8 m je rad/s   -> Default 0,2 rad/s = 12 px, Maximum 2,0 rad/s = 120 px
// Der Motor rechnet den Faktor auf Grad-Einheiten (L[m] = Wert[°]·Faktor),
// daher die Umrechnung ·pi/180. Die Kappe (in m) verhindert, dass ein großer
// \omega-Pfeil bei extremen Reglerwerten aus der Zeichenflaeche laeuft; sie
// greift erst jenseits von 3,25 rad/s. \vec r und \vec v nutzen die echten
// Laengenskalen des Motors (currentPixelsPerMeter bzw. VEL_SCALE) — keine
// Overrides noetig.
...
// = 1,73, bei 60° nur noch um 1,15. Genau diese perspektivische Laengenaenderung
// wurde als Fehler gemeldet („die Bahngeschwindigkeit wird laenger/kuerzer"),
// obwohl |\vec v| = |\omega|R konstant ist. Statt sie zu verstecken, ist sie hier
// UMSCHALTBAR: Default „flach" (60°, Laengen fast massstaeblich lesbar),
// Alternative „raeumlich" (die Isometrie der Quell-Sim und der Legacy-Ansicht
// „Iso"). Der Vergleich beider Stellungen zeigt, dass die Aenderung von der
// Perspektive kommt und nicht von der Physik — die Live-Analyse und das Diagramm
// bleiben in beiden Stellungen identisch. Preis des flachen Blicks: die Drehachse
// wird staerker verkuerzt (Faktor 0,61), \vec\omega also kuerzer gezeichnet.
...
//
//    PFEILSPITZEN: wie in 1.59 markerUnits="userSpaceOnUse" mit FESTER Groesse
//    (s. aspekt_alpha_omega.js fuer die Begruendung der refX-Werte). Der
//    \alpha-Marker (ks_arrow-alpha) bleibt definiert, wird aber nicht benutzt
//    (\alpha=0 -> drawVectors zeichnet keinen \alpha-Vektor).
const SVG_SCENE = `
<svg id="ks_main_svg" viewBox="0 0 400 480" preserveAspectRatio="xMidYMid meet" class="aspekt-svg">
  ...
  <g id="ks_animation_group">
    ...
    <!-- \vec\omega auf der z-Achse. \vec\alpha entfaellt hier (\alpha=0 fix); der
         Stub bleibt, drawVectors() zeichnet ihn nur, wenn alpha_rad !== 0. -->
...
//    EINZEL-Modus (diagramMode='1', graphType1='phi'); Slot 2 bleibt verborgen,
//    ist aber als Stub vorhanden, weil drawGraphs()/hideGraphHover(2) darauf
//    zugreifen. Aufbau wie 1.59 (nur die gezeichnete Groesse ist \varphi, nicht
//    \omega — der Motor entscheidet das anhand von graphType1).
const SVG_GRAPH = `
...
// -- Linkes Bedien-Panel — Abschnitte einklappbar (Muster wie 1.50/1.51/1.59). --
...
<div class="aspekt-panel aspekt-panel-left">
  ...
  <div class="panel-section collapsible collapsed">
    ...
    <div class="aspekt-hint">Die Bahnebene wird schräg gesehen, der Kreis erscheint daher als Ellipse — und ein Pfeil in dieser Ebene je nach Richtung unterschiedlich lang, obwohl sich sein Betrag nicht ändert. „räumlich" ist die echte isometrische Ansicht (Längen schwanken um den Faktor \\(\\sqrt3\\approx1{,}73\\)), „flach" blickt steiler von oben (nur noch 1,15). Beim Umschalten ändern sich <em>nur</em> die Perspektive — Live-Analyse und Diagramm bleiben gleich, \\(|\\vec v|=|\\omega|R\\) ist in beiden Stellungen konstant. Dafür wird im flachen Blick die Drehachse stärker verkürzt, \\(\\vec\\omega\\) also kürzer gezeichnet.</div>
    <div class="aspekt-hint">\\(\\vec v=\\vec\\omega\\times\\vec r\\) (Rechte-Hand-Regel): \\(\\vec\\omega\\) auf der Drehachse, \\(\\vec r\\) in der Ebene -> \\(\\vec v\\) tangential. \\(\\vec\\omega\\) [rad/s], \\(\\vec r\\) [m] und \\(\\vec v\\) [m/s] haben verschiedene Einheiten — ihre Pfeillängen sind jeweils proportional zum eigenen Wert, aber untereinander nicht vergleichbar. Aussagekräftig ist die RICHTUNG (und bei \\(\\vec v\\) der reale Betrag \\(|\\vec v|=|\\omega|R\\)).</div>
  </div>
  <div class="panel-section collapsible collapsed">
    ...
    <div class="legend-grid">
      ...
      <div class="legend-swatch" data-c="rlat"></div><div class="legend-label">Ortsvektor \\(\\vec{r}\\)</div>
      <div class="legend-swatch" data-c="vlat"></div><div class="legend-label">Bahngeschw. \\(\\vec v=\\vec\\omega\\times\\vec r\\)</div>
...
// -- Klebende Ablaufleiste oberhalb von Szene + Diagramm (wie 1.44/1.51/1.59). --
const RUNBAR = `
<div class="aspekt-runbar" role="group" aria-label="Ablaufsteuerung">
  <div class="aspekt-btn-row">
    ...
    <button type="button" class="aspekt-btn aspekt-btn-icon" data-act="stop" aria-label="Pause: Ablauf anhalten" title="Pause"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="7.5" y="5.5" width="3.4" height="13" rx="1.5" fill="currentColor"/><rect x="13.1" y="5.5" width="3.4" height="13" rx="1.5" fill="currentColor"/></svg></button>
  ...
  <div class="panel-body">
    ...
    <div class="panel-section">
      ...
      <div class="formula-box">
        <div class="formula-box-cap">Bahngeschwindigkeit als Kreuzprodukt:</div>
        <div>\\[\\vec v(t) = \\vec\\omega \\times \\vec r(t)\\]</div>
        <div>\\[\\vec\\omega = \\begin{pmatrix}0\\\\0\\\\\\omega_0\\end{pmatrix},\\quad \\vec r(t)=\\begin{pmatrix}R\\cos(\\omega_0 t)\\\\R\\sin(\\omega_0 t)\\\\0\\end{pmatrix}\\]</div>
        <div class="formula-box-cap">Vorzeichen von \\(\\omega_0\\) legt die Drehrichtung fest (Rechte-Hand-Regel): \\(\\omega_0&gt;0\\) gegen den Uhrzeigersinn, \\(\\omega_0&lt;0\\) im Uhrzeigersinn.</div>
      </div>
    </div>
  </div>
</div>`;
...
// ── Factory: baut EINE omega-vektor-Aspekt-Figur mit eigener Motor-Instanz ────
export function buildOmegaVektorFig(fig) {
    ...
    // Per-Instanz-Regler + Zustand (Closure, nicht Modul-Ebene). Kein \alpha-
    // Regler: \alpha ist hier stets 0 (gleichfoermige Kreisbewegung).
    ...
    // -- Vergleichskurve (Geisterzug): \varphi(t) des letzten Parametersatzes
    //    einfrieren und ueber dem neuen Durchlauf stehen lassen (nur die jeweils
    //    letzte, keine Ansammlung). Bei \omega_0-Vorzeichenwechsel legt sich die
    //    fallende Rampe exakt auf die steigende (gleicher |Betrag|) — die Kern-
    //    aussage der Abbildung (Drehrichtung = Vorzeichen von \omega_0).
    //
    //    Gespeichert werden DATEN (t, \varphi in Grad — die Rohreihe des Motors),
    //    NICHT Pixel: die y-Achse skaliert automatisch mit dem Wertebereich
    //    (\omega_0 aendert ihn). Ein in Pixeln eingefrorener Zug bliebe stehen,
    //    waehrend die Achse unter ihm wegskaliert. Deshalb wird er bei jedem
    //    Zeichnen aus den Rohdaten neu auf die AKTUELLE Skala projiziert
    //    (store.graphScale[1], vom Motor beim Zeichnen gesetzt). Analog zu 1.59,
    //    nur dass die Rohreihe p_phi (statt p_omega) eingefroren wird.
    const prevLine = ge(p + 'graph_prev_line');
    let prevSeries = null;                // {t:[…], phiDeg:[…]}
    function snapshotPrev() {             // inside withStore aufrufen
        const fd = store.fullData;
        prevSeries = (fd && fd.t && fd.t.length)
            ? { t: fd.t.slice(), phiDeg: fd.p_phi.slice() }
            : null;
    }
    function clearPrev() {
        prevSeries = null;
    ...
    function renderPrev() {               // inside withStore aufrufen
        ...
        for (let i = 0; i < prevSeries.t.length; i++) {
            ...
            const py = plotT + plotH - ((prevSeries.phiDeg[i] * cf - valMin) / yR) * plotH;
    ...
    function draw(t) {
        ...
        // Hover-Cursor dauerhaft bei t = 0 (Fallstrick #23).
    ...
    function updateLabels(t) {
        ...
        // Drehrichtung — der Kern der Abbildung: das Vorzeichen von \omega_0
        // legt sie fest (Rechte-Hand-Regel).
        ...
        if (hint) {
            hint.textContent = Math.abs(omega0) < 1e-9
                ? 'ω₀ = 0: Stillstand, ϕ bleibt konstant.'
                : (omega0 > 0
    ...
    //    (\omega_0, R): dann wird — wenn „Letzte Kurve behalten" aktiv ist —
    //    die eben gezeigte Kurve als Geisterzug eingefroren und die Laufzeit auf
    //    0 zurueckgesetzt, damit der neue Verlauf von vorn ueber dem alten entsteht
    //    (Konvention der Stand-alone-Sims, Fallstrick #20).
    function rebuild(paramChange = false) {
        rt.withStore(() => {
            ...
            Object.assign(store, {
                ...
                alpha_rad: 0,
                // Diagramm: EIN Slot, \varphi(t)
                diagramMode: '1', graphType1: 'phi',
                // Sichtbarkeiten: \omega + \vec r + \vec v immer; \varphi/Bahn per Kasten
                ...
                togVelocity: true,
                // \vec a gehoert zu Abb. 1.58, hier bewusst aus
                togAcceleration: false,
                ...
                simDuration: T_AUTO,
                // Eigener Laengen-Maßstab des \omega-Vektors (s. oben); \vec r/\vec v
                // nutzen die echten Skalen des Motors (keine Overrides).
    ...
    function onInput(e) {
        ...
        // beendet die Geste (Fallstrick #18).
        if (!paramGesture) {
            paramGesture = true;
