// aspekt_betraga_zeit.js — interaktive Aspekt-Figur zu Abbildung 1.47 (1.4.6
// „Beschleunigung …", Betrag-der-Bahnbeschleunigung-Diagramm). Zeigt die
// Kreisbahn-Szene mit Ortsvektor r und Beschleunigungsvektor a (radial,
// antiparallel zu r) sowie rechts daneben das |a|(t)-Diagramm — der Betrag
// |a| = ω²·R ist bei konstantem Radius und konstanter Periode T KONSTANT
// (waagerechte Linie), obwohl der Vektor a selbst ständig seine Richtung
// ändert. Genau dieser Kontrast (Vektor dreht sich, Betrag bleibt gleich) ist
// der Aspekt — die Beschleunigung existiert trotz konstantem |v|, weil sich
// die RICHTUNG von v ständig ändert.
// Regler: Zeit t (0 … 12 s), Radius R (R geht linear in |a| ein, ändert also
// die Höhe der Linie), Periodendauer T (→ ω = 2π/T).
//
// TECHNIK: kein eigener Zeichencode (keine Szenen-Zusatzzeichnung wie der
// Winkelbogen in 1.41 — a wird komplett vom Motor gezeichnet). Der Motor der
...
// Aspekt. EINZELNES Diagramm mit graphType1='aabs' (nicht gestapelt wie 1.46):
// der Motor liefert aabs nativ (physics.js, |R·ω²| pro Zeitschritt — konstant).
...
// updateGraph() zeichnen daraus Punkt, Ortsvektor, Beschleunigungsvektor,
// Bahnspur, die (konstante) |a|(t)-Linie — die optionale Anim-Schleife fährt
// die Zeit von 0 bis 12 s (Slow-Mo via Tempo-Pills).
//
// VORLAGE ist aspekt_betragv_zeit.js (Abb. 1.43, EINZELNES Diagramm + Play/
// Pause + Ghost-Linie): gleiche Interaktion (Szene neben einzelnem Diagramm),
// nur der Aspekt wechselt von Geschwindigkeitsbetrag auf Beschleunigungs­
// betrag. Der Winkelbogen (1.41-spezifisch) entfällt ersatzlos, der
// Beschleunigungsvektor (statt Geschwindigkeitsvektor) kommt hinzu; r bleibt
// sichtbar, weil a⃗ antiparallel zu r⃗ ist — der paedagogische Punkt dieser
// Figur. Was aus der Vorlage stammt, lief auf Anhieb; deshalb wurde kopiert
// und feature-gated, nicht neu erfunden (Runbook Abschnitt 0a).
//
...
// 1.39/1.41/1.43 — NICHT die neuere markerUnits=strokeWidth-Variante aus 1.42/
// 1.46, damit diese Figur intern konsistent mit ihrer eigenen Vorlage bleibt),
// damit render.js' Verkürzung die Spitze exakt auf den Zielpunkt setzt.
// Strichstärken nicht ändern. |a|(t)-Kurve in --kb-acc (der Beschleunigungs-
// vektor-Farbe), damit Szene und Diagramm zusammengehören.
//
// LAYOUT: wie 1.43 — die Kreisbahn-Szene steht NEBEN dem |a|(t)-Diagramm
// (Zeile) — wie die statische Vorlage. Nur im Schmal-Modus wird gestapelt
// (Szene über Diagramm); breit + Lupe ebenfalls Zeile (s. aspekt_betraga_zeit.css).
//
...
const T_AUTO = 12;            // fester Auto-Stopp nach 12 s — bei Vorgabe T=4 s
                              // sind das drei Perioden (Bereich 0…12 s, wie 1.39/1.43)
...
const R_DEFAULT = 1.5;        // R ist hier ein Regler (anders als 1.41): |a|=ω²·R
const ANIM_CX = 225, ANIM_CY = 260;   // = ANIM_CX / ANIM_CY_STACK (render.js)

// -- Szene: exakt die Vorlagen-Geometrie (wie aspekt_betragv_zeit, MIT Winkelbogen
//    und foreignObject-φ-Label, OHNE Stoppuhr — der Aspekt ist der Beschleunigungs-
//    betrag, nicht die Zeit als Uhr). Marker mit fester Länge (s. aspekt_kreisbahn.js
//    für die Herleitung). Der Beschleunigungsvektor (radial, antiparallel zu r)
//    ersetzt den Geschwindigkeitsvektor der Vorlage; r bleibt sichtbar. Versteckte
//    Stubs (v/a-Komponenten, Stoppuhr), die updateScene() anfasst; die kb_-IDs
//    werden pro Instanz durch den Prefix ersetzt.
const SVG_SCENE = `
<svg id="kb_main_svg" viewBox="0 0 450 480" preserveAspectRatio="xMidYMid meet" class="aspekt-svg">
  <defs>
    ...
    <!-- Beschleunigungsvektor: gleiche Geometrie wie kb_arrowhead-r (beides
         "Haupt"-Vektoren, ARROW_LEN_MAIN=12.5 in render.js) — kein eigener
         Massstab noetig, da diese Figur (anders als 1.46) a ohne Komponenten
         zeigt und daher bei derselben Strichstaerke wie r bleibt. -->
    <marker id="kb_arrowhead-a"  markerUnits="userSpaceOnUse" markerWidth="18.75" markerHeight="13.125" refX="6.25" refY="6.5625" orient="auto"><polygon points="0 0, 18.75 6.5625, 0 13.125"/></marker>
  ...
  <g id="kb_animation_group">
    ...
    <!-- Beschleunigungsvektor (radial, antiparallel zu r) = die gezeigte Groesse.
         Keine Komponenten (a_x/a_y sind 1.46s Aspekt, nicht dieser) -- Stubs
         bleiben als leere Linien vorhanden, damit updateScene() sie dereferenzieren
         kann. -->
    <line id="kb_acceleration_vector"   stroke-width="2.5" marker-end="url(#kb_arrowhead-a)" visibility="hidden"/>
    <line id="kb_acceleration_vector_x"/><line id="kb_acceleration_vector_y"/>
...
//    den EINZELN-Modus (graph_group_single, graphType1='aabs'); die gestapelten
...
//    Wie 1.43: R ist hier ein Regler (nicht fest), da |a|=ω²·R von R abhängt —
//    R ändert also die Höhe der |a|(t)-Linie.
const PANEL_LEFT = `
...
//    kompakt und NUR auf diese Figur gemünzt (|a|(t), R, ω), UNNUMMERIERT,
  ...
  <div class="panel-body">
    ...
    <div class="panel-section">
      ...
      <div class="formula-box">
        <div class="formula-box-cap">Bahnbeschleunigung auf der Kreisbahn:</div>
        <div>\\[\\vec{a}(t) = \\omega(t)^{2}\\,R\\,\\begin{pmatrix}-\\cos(\\varphi(t)) \\\\ -\\sin(\\varphi(t))\\end{pmatrix}\\]</div>
        <div>\\[|\\vec{a}(t)| = \\omega(t)^{2}\\,R\\]</div>
      </div>
    </div>
  </div>
</div>`;
...
// ── Factory: baut EINE Betrag-a-Zeit-Aspekt-Figur mit eigener Motor-Instanz ────
export function buildBetragAZeitFig(fig) {
    ...
    // -- Vergleichslinie (Ghost): die |a|(t)-Linie des letzten Parametersatzes
    //    einfrieren (gestrichelt + dünner, s. aspekt_betraga_zeit.css) und über
    ...
    //
    //    Gespeichert werden DATEN (t/|a|), nicht Pixel: die y-Achse skaliert mit
    //    R und ω (|a|=ω²·R). Eine in Pixeln eingefrorene Polyline würde beim
    ...
    let prevSeries = null;                // {t:[…], aabs:[…]} — komplette Kurve
    function snapshotPrev() {             // inside withStore aufrufen
        prevSeries = store.tData.length
            ? { t: store.tData.slice(), aabs: store.aabsData.slice() }
            : null;
    }
    function clearPrev() {
        prevSeries = null;
    ...
    function renderPrev() {
        ...
        for (let i = 0; i < prevSeries.t.length; i++) {
            ...
            const py = plotT + plotH - ((prevSeries.aabs[i] - yMin) / yR) * plotH;
    ...
    //    durchlaufenen Winkelbogen, auch wenn der Aspekt hier |a| statt φ/ω
    ...
    // -- Zeitreihe auf feste 12 s begrenzen (3 Perioden bei T=4 s, wie 1.39/1.41/1.43).
    ...
    function precomputeRange(duration) {
        ...
        // (anderes R/T -> anderes |a|) oben aus dem Diagramm heraus und der
        // Vergleich bricht ab.
        if (prevSeries && prevSeries.aabs.length) {
            const lim = store.axisLimits.aabs;
            const need = Math.max(...prevSeries.aabs) * 1.1;
    ...
    function updateLabels(t, T) {
        ...
        if (vt) {
            ...
            ge(p + 'ak_val_aabs').textContent = n(Math.abs(store.R * store.omega * store.omega), 2) + ' m/s²';
        }
    }
    ...
    function rebuild(paramChange = false) {
        rt.withStore(() => {
            ...
            Object.assign(store, {
                isStacked: false, graphType1: 'aabs',
                ...
                showAccelerationVector: true, showAccelerationComponents: false,
