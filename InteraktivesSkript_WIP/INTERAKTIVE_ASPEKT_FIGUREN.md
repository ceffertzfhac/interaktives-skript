...
- **Abbildung 1.57** — `src/figures/aspekt_omega_vektor.js` + `.css`
  (Winkelgeschwindigkeit als Vektor in 1.4.8, \vec v = \vecω × \vec r; Regler
  t, R, ω₀; α=0 fix; φ(t)-Graph). Klon von 1.59 auf demselben `kreis_spiral`-
  Motor — die nächste ISO-3D-Figur kopiert am besten **hieraus** (oder aus 1.58).
  Standard ω₀ = 1,0 (s. Fallstrick #26 — bei 0,2 war die maßstäbliche
  Bahngeschwindigkeit \vec v unsichtbar; nachträglich korrigiert).
- **Abbildung 1.58** — `src/figures/aspekt_zentripetalkreuz.js` + `.css`
  (Zentripetalbeschleunigung als Kreuzprodukt in 1.4.8, \vec a_ZP = \vecω ×
  \vec v; Regler t, R, ω₀; α=0 fix; |a_ZP|(t)=Rω²-Graph, KONSTANT). Klon von
  1.57; Standard ω₀ = 2,0 bei Reglerbereich ±4 (Nutzervorgabe; s. Fallstrick #26
...
| 24 | Ein Vektor der Vorlagen-Sim ist in der Aspekt-Figur praktisch unsichtbar (α ~17 px gegen ω ~189 px) | Die Längen-Faktoren der Sim (`OMEGA_LEN_FACTOR`, `ALPHA_LEN_FACTOR`) sind auf **ihre** Reglerbereiche (°/s bzw. °/s²) abgestimmt. In den Bereichen der Figur (rad, kapitelkonform) ergibt derselbe Faktor ein Längenverhältnis von ~11:1 — und damit ist die Bildaussage („α parallel/antiparallel zu ω") verloren | Größen mit **verschiedenen Einheiten** haben keinen gemeinsamen Maßstab: jeder Größe seinen **eigenen, linearen** Maßstab geben (proportional zum Wert, doppelter Wert = doppelter Pfeil), im Motor additiv überschreibbar machen (`store.omegaLenFactor`/`.alphaLenFactor`/`.axisVecLenCap`) und die Konvention **in der Bildunterschrift benennen**. Eine Längen-Kappe verhindert, dass der Pfeil bei Extremwerten aus der Zeichenfläche läuft (vgl. die log-Skalierung in 1.50/1.51) |
| 25 | Falsches „Abb. 1.n"-Badge in der interaktiven Bildunterschrift (zeigt die Nummer einer *anderen* Abbildung) | Die statische Abbildung teilt sich ihre `id` mit einem **früheren** Vorkommen derselben Vorlage (z. B. 1.56 und 1.57 heißen beide `…dphi-dr`). `label_aspekt_figuren` löst `data-figref` per `getElementById` → **erstes** Vorkommen gewinnt → Badge der *früheren* Abbildung. Real bei 1.57: Badge hätte „1.56" gezeigt statt „1.57" | Dem **zweiten** Vorkommen eine **eindeutige** `id` geben (`…omegavecpos`), `data-figref` darauf zeigen lassen; das erste Vorkommen behält die alte `id` (dessen Selbst-Xref „Abbildung 1.56" bleibt korrekt). Vorab grep: wird irgendwo (Prose/Xref) auf die zweite `id` referenziert? Die Nummerierung zählt nach **Dokumentreihenfolge** → umbenennen verschiebt nichts |
| 26 | Ein Vektor ist in der Aspekt-Figur **unsichtbar**, obwohl er im statischen Vorbild dominant war (a_ZP ~1 px) | Das statische Vorbild zeichnet Vektoren **übertrieben** („stark übertrieben … infinitesimal klein"); die Aspekt-Figur zeichnet sie **maßstäblich** (reale Länge, `VEL_SCALE`/`ACC_SCALE`). Bei der familienweiten Default-ω₀ = 0,2 ist \|a_ZP\| = R·ω² = 0,06 m/s² → ~1 px. **Der Motor blendet zu kurze Vektoren ganz aus**, nicht nur klein: `setMain` → `shortenEnd(...)` liefert `null`, sobald die Pfeillänge ≤ der Pfeilspitze ist (`ARROW_LEN_MAIN` = 12,5 px) → `visibility:hidden`, ein hartes Verschwinden ohne Vorwarnung. Real bei 1.58 — und **erneut bei 1.57** (\vec v = ω₀R = 0,3 m/s → 0,3·`VEL_SCALE`·75 px/m ≈ 9 px < 12,5 → \vec v war im Ausgangszustand gar nicht zu sehen; als Bugreport gemeldet, Default auf 1,0 angehoben) | Default **pro Figur** wählen, sodass *jeder* gezeigte Vektor lesbar ist (1.58: ω₀ = 2,0 → \|a_ZP\| ~90 px; 1.57: ω₀ = 1,0 → \|v\| ~45 px) — und die **Abweichung vom Familien-Default im Modulkopf und in der Bildunterschrift begründen**. Wer den Vektor kleiner sehen will, regelt ω₀ herunter. **Gegenprobe vor dem Abhaken**: `figur_screenshot.mjs --measure='line[id$=<vektor>]' --ink` im Ausgangszustand — eine Ink-Box `w=0,h=0` heißt „ausgeblendet", nicht „klein" |
| 27 | „Die Bahngeschwindigkeit wird länger und kürzer, obwohl \|v\| konstant ist" (Bugreport zu 1.57) | **Kein Fehler, sondern die ISO-Projektion.** Die Bahnebene wird schräg gesehen, der Kreis also als Ellipse mit Achsverhältnis √3 — ein Vektor konstanter Länge *in dieser Ebene* projiziert je nach Richtung auf 0,707× … 1,225×, Faktor 1,73. Betrifft \vec r genauso wie \vec v (gemessen 72 → 122 px). **Vor dem „Fix" gegenprüfen**: `projectISO` ist algebraisch identisch mit Legacy `to2d(…, perspective 3)` (`script.js:215`) *und* mit der Quell-Sim (`diff` über render.js) — nur stehen beide per Default in der Draufsicht („Top" bzw. `currentView:'2D'`), 1.57–1.59 dagegen dauerhaft in ISO | Nicht die Geometrie verbiegen (Vektoren auf konstante Bildschirmlänge normieren wäre gelogen), sondern die **Blickhöhe umschaltbar** machen: `store.isoElevation` (Grad, undefined = Isometrie der Quelle), Pills „flach" (60°, Default) / „räumlich" in „Darstellung". Bei 60° bleiben nur 1,15 Schwankung; der Preis ist die stärker verkürzte Drehachse (0,61 → ω/α kürzer). Der Vergleich beider Stellungen macht den Effekt selbst zum Lerngegenstand. `projectISO` hält den WAAGERECHTEN Ebenenfaktor fest, sonst müsste die Zoom-Regel jeder Figur nachgezogen werden |
