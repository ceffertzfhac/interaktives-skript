...
- **Versionierung `#header_version` (Nutzervorgabe, ab v1.15.1)**: dreistellig `MAJOR.MINOR.PATCH`. **Neuer Abschnitt / größeres Feature → MINOR-Stelle** hochzählen (`1.15 → 1.16 → 1.17 …`), **Kleinänderung (Fix, Text/Optik-Anpassung, Doku) → PATCH-Stelle** (`1.15 → 1.15.1 → 1.15.2 …`). MAJOR bleibt vorerst `1`. Die Version steht als einzige Quelle in `index.html` (`#header_version`) und wird pro Commit-Einheit passend zur Änderungsart mitgezogen. (Historisch bis v1.15 wurde die MINOR-Stelle auch für Kleinänderungen genutzt; die feinere Patch-Stufe gilt erst *ab jetzt*.)
